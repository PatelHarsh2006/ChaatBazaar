...
const NAVBAR_HTML = `
  <div class="nav-shell">
    <a class="nav-brand" href="index.html" aria-label="ChaatBazar Home">
      <span class="nav-brand-mark">ChaatBazar</span>
      <span class="nav-brand-tag">Street Food</span>
    </a>
    ...
    <div class="nav-collapse" id="primary-nav">
      <nav class="nav-links" role="navigation" aria-label="Primary">
        ...
        <a class="nav-link" data-nav="menu" href="menu.html">Menu</a>
        <a class="nav-link" data-nav="recommendations" href="recommendations.html">Smart Combos</a>
        <a class="nav-link" data-nav="favorites" href="favorites.html">
          Favorites
          <span class="nav-count" id="fav-count">0</span>
        </a>
        <a class="nav-link" data-nav="orders" href="orders.html">Orders</a>
        <a class="nav-link" data-nav="about" href="about.html">About</a>
        <a class="nav-link" data-nav="faq" href="faq.html">FAQ</a>
      </nav>

      <div class="nav-actions" role="group" aria-label="Account">
        <a class="nav-link nav-pill" data-nav="cart" href="cart.html">
          Cart
...
function renderNavbar() {
  ...
  headerInner.innerHTML = NAVBAR_HTML;
...
function setActiveNavLink(container) {
  ...
  const activeKey = isHome ? "home" : activeMap[currentPage];
  if (!activeKey) return;
...
function setupNavToggle(container) {
  const header = container.closest(".site-header");
  const toggle = container.querySelector(".nav-toggle");
  const collapse = container.querySelector(".nav-collapse");
...
function updateAuthActions(container) {
  ...
  actions.innerHTML = `
    ...
    <a class="nav-link nav-ghost" data-nav="profile" href="profile.html">Profile</a>
    <a class="nav-link nav-ghost" data-nav="dashboard" href="dashboard.html">Dashboard</a>
    <button class="nav-link nav-ghost nav-button" type="button" data-nav="logout">
      Logout
    </button>
  `;

  const logoutBtn = actions.querySelector("[data-nav='logout']");
  if (logoutBtn) {
...
function applyAllFilters() {
  ...
  setTimeout(() => {
    ...
    if (minRating !== "All") {
      filtered = filtered.filter(
        (item) => (item.rating || 5) >= parseFloat(minRating),
      );
    }
...
window.reorderOrder = function (orderId) {
  ...
  updateFavCount();
  renderCart();
  showToast("Items added back to your cart successfully!");
};
...
function setupContactForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const validation = validateAndSanitizeContactForm(
      nameInput.value,
      emailInput.value,
      messageInput.value,
    );
    ...
    if (!validation.valid) {
      ...
      if (validation.errors.message)
        errorMessage.textContent = validation.errors.message;
      return;
    }
...
function applyUrlFilterParam() {
  ...
  applyAllFilters();
  // If the URL included the '#filters' anchor, scroll to the toolbar.
